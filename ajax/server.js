...
var server = http.createServer(function(request, response){
  ...
  if(path === '/'){
    ...
    const json = fs.readFileSync('db/page1.json');
    let jsonArr = JSON.parse(json);
    let newPage = jsonArr.map(item => `<li>${item.id}</li>`)
    // console.log(json.toString());
    // console.log(newPage.join(''));
    string = string.replace('{{page1}}',`<ul>${newPage.join('')}</ul>`);
    response.write(string)
    response.end()
  } else if(path === '/3.html'){
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/html;charset=utf-8')
    const string = fs.readFileSync('public/3.html');
    response.write(string)
    response.end()
    ...
    const string = fs.readFileSync('public/4.xml');
    response.write(string)
    response.end()
  } else if(path === '/5.json'){
    response.statusCode = 200
